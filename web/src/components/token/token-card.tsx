      ...
      <Card size="sm" className={cn("h-full transition-all hover:-translate-y-0.5 hover:ring-foreground/20", token.graduated && "ring-gold/40")}>
        <CardContent className="space-y-3">
          <div className="flex items-start gap-3">
            <TokenAvatar logo={token.logo} symbol={token.symbol} seed={token.address} size={44} />
          ...
          <div className="grid grid-cols-3 gap-2 text-[11px] text-muted-foreground">
            ...
            <div className="text-right">
              ...
              <div className="font-mono text-xs text-foreground tabular">{fmtNum(token.holders ?? 0)}</div>
            </div>
          </div>
