...
import { usd, useConfig, useTreasury } from "@/lib/api";
import { fmtUsd } from "@/lib/format";
import { EXPLORER, addrUrl, chain } from "@/lib/web3";
import { X_HANDLE, X_URL } from "@/lib/site";
import { useApp } from "@/components/providers";
import { Badge } from "@/components/ui/badge";
import { Card, CardContent } from "@/components/ui/card";
import { Addr } from "@/components/shared";
import { CtoForm } from "@/components/cto-form";
...
export default function DocsPage() {
  ...
  const treasury = useTreasury().data;
  const p = cfg?.params;
      ...
      <div className="grid gap-6 lg:grid-cols-[200px_1fr]">
        ...
        <div className="min-w-0 space-y-10">
          ...
          <Section id="overview" title={L("概览", "Overview")}>
            <P>
              {L(
                "ArcLaunch 是为 Circle Arc 公链打造的代币发射与交易平台。你可以浏览发射、打开任意代币查看详情，并直接用钱包交易。所有价格都以 USDC 计价——在 Arc 上 USDC 既是 gas 也是交易货币，你只需要一种资产。",
            ...
            <P>
              <strong>{L("ArcLaunch 从不托管你的资金。", "ArcLaunch never holds your funds.")}</strong>{" "}
              {L(
                "每一次代币发射和交易都是你的钱包签名的链上交易；用户本金始终在 Uniswap V3 官方字节码的池子里，ArcLaunch 自研合约只经手手续费流。",
                "Every launch and trade is a transaction your wallet approves. User principal always sits in pools running official Uniswap V3 bytecode; ArcLaunch's own contracts only ever touch the fee stream.",
              )}
            </P>
            <Facts
          ...
          <Section id="launch" title={L("代币发射流程", "How launches work")}>
            ...
            <Facts
              ...
              items={[
                ...
                L("管理员可以调整这个数值，但只影响之后新发的代币。", "The admin can change this value; it only affects launches after the change."),
              ]}
            />
            <KvGrid
              rows={[
          ...
          <Section id="tax" title={L("税币模式", "Tax mode")}>
            ...
            <Facts
              items={[
                L("协议从税里拿 0；协议收入仍只来自池费的 0.25% 和创建费。", "The protocol takes 0 of the tax; protocol revenue is still only the 0.25% pool-fee share and creation fees."),
                ...
                L("对比 dyorswap：它的税币只能迁到 Uniswap V2、收款钱包固定为发射钱包；ArcLaunch 的税币从第一秒就在 V3 池里交易，营销 / 团队两个地址可自由填写（多签、冷钱包均可），分红档暂不提供。", "Versus dyorswap: its tax tokens can only migrate to Uniswap V2 and pay the deployer wallet; ArcLaunch tax tokens trade in a V3 pool from the first second, the marketing / team wallets are free-form (multisig or cold wallet), and a holder-dividend option is not offered."),
              ]}
            />
          </Section>
          ...
          <Section id="revenue" title={L("协议收入", "Protocol revenue")}>
            <P>
              {L(
                "每笔交易 1% 池费的分配：项目方 / 创作者 75%、ARCL 回购销毁 5%、储备金 19%、开发团队 1%。创作者的 0.75% 由 FeeLocker 直接打到创作者钱包；其余 0.25% 进入自动结算合约，每 7 天结算一次（任何人可调用 execute()，Keeper 到点自动触发）：0.19% 转入储备金多签，0.01% 转入开发团队地址，0.05% 划入回购金，随后分批在 $ARCL 的池子里买入并转入 0x…dEaD 销毁地址（Arc 禁止转零地址）。每笔回购限价涨幅 1.5%，间隔 10 分钟，成交价不低于 10 分钟 TWAP 的 90%——从合约层面严防 MEV 夹击。平台币未配置期间，回购部分留在合约里累积，配置后开始买入销毁。创建费直接进入储备金多签。平台不设资金池：所有收入的终点只有多签、开发团队地址和销毁地址。比例、7 天周期、两个收款地址都写死在合约里不可更改。",
            ...
            <KvGrid
              rows={[
                [L("项目方 / 创作者", "Project / creator"), L("每笔 1% 池费的 75%（0.75%），FeeLocker 直付，毕业前后不变", "75% of the 1% fee (0.75%), paid directly by the FeeLocker, unchanged after graduation")],
                [L("ARCL 回购销毁", "ARCL buyback & burn"), L("每笔 1% 池费的 5%（0.05%），每 7 天划入回购金后分批买入（≤1.5% 涨幅 / 10 分钟一批 / TWAP 90% 地板）", "5% of the 1% fee (0.05%), reserved every 7 days and bought in slices (≤1.5% move / 10-min cooldown / 90% TWAP floor)")],
                [L("储备金（多签）", "Reserve (multisig)"), L("每笔 1% 池费的 19%（0.19%）+ 全部创建费：创作者激励、运营、审计", "19% of the 1% fee (0.19%) + all creation fees: creator incentives, operations, audits")],
            ...
            />
            <P className="text-muted-foreground">{L("销毁不保证价格上涨。", "Burning does not guarantee a higher price.")}</P>
          </Section>

          {/* ---------------------------------------------------------- cto */}
          <Section id="cto" title={L("社区接管", "Community takeovers")}>
            <P>{t("cto.desc")}</P>
            <Steps
              steps={[
                [L("申请", "Request"), L("社区通过下方表单提交申请（钱包签名，不上链）。", "The community submits the form below (wallet-signed, off-chain).")],
                [L("提案", "Propose"), L("团队审核通过后，管理员在链上调用 proposePayout()，进入 48 小时等待期。", "If approved, the admin calls proposePayout() onchain, starting a 48-hour delay.")],
                [L("否决或执行", "Veto or execute"), L("等待期内原创作者可一键否决；期满后任何人可执行，收款地址切换。", "The original creator can veto during the delay; afterwards anyone can execute and the payout switches.")],
              ]}
            />
            <P className="text-muted-foreground">
              {L(
                "与 pons 不同：pons 的团队可以直接改收款地址；ArcLaunch 的管理员做不到即时修改，48 小时时间锁和创作者否决权写在合约里。审批是运营行为，不代表对代币安全性或价值的任何背书。",
                "Unlike pons, where the team changes the payout directly, the ArcLaunch admin cannot change it instantly: the 48-hour timelock and the creator's veto are enforced by the contract. Approval is administrative and is not an endorsement of a token's safety or value.",
              )}
            </P>
            <Card size="sm">
              <CardContent>
                <div className="mb-3 font-semibold">{t("cto.title")}</div>
                <CtoForm />
              </CardContent>
            </Card>
          </Section>
          ...
          <Section id="risk" title={L("风险披露", "Risk disclosures")}>
            <P>
              {L(
                "通过 ArcLaunch 发射的代币由用户创建，属于实验性资产。签名前请核对代币地址、创作者、流动性、持有人集中度和交易预览。",
            ...
            />
            <P className="text-muted-foreground">{L("ArcLaunch 只是一个界面，不构成投资建议，也不代表对任何代币质量的判断。", "ArcLaunch is an interface, not investment advice or a representation of token quality.")}</P>
          </Section>
          ...
          <Section id="integration" title={L("集成指南", "Integration")}>
            ...
            <KvGrid
              rows={[
                [L("网络", "Network"), `Arc ${t("common.testnet")}`],
...
// pending community takeover, if any
const [newPayout, eta] = await client.readContract({ address: locker, abi: lockerAbi, functionName: "pendingPayout", args: [token] });`}</Code>
  ...
  "function taxConfig() view returns (uint16 buyBps, uint16 sellBps, address marketing, address team, uint16 marketingShareBps)",
            ...
            <P>
              {L(
                "ArcLaunch 自己的索引器也对外开放，方便快速接入（非权威源，以链上为准）：",
            ...
            <KvGrid
              rows={[
                ...
                ["GET /api/stats · /treasury · /health", L("平台统计、结算与销毁、同步状态", "Platform stats, settlement & burns, sync state")],
                ["WS /ws", L("trade / launch / fees / graduated / burn 实时推送", "Live trade / launch / fees / graduated / burn events")],
              ]}
            />
            {treasury?.platformToken && (
              <>
                <H3>{L("参考代币", "Reference token")}</H3>
                <P>{L("$ARCL 是通过同一个 Factory 发出的平台币，可用来校验你的索引器。", "$ARCL is the platform token, launched through the same factory — use it to validate an indexer.")}</P>
                <KvGrid
                  rows={[
                    ["Token", <AddrLink key="pt" a={treasury.platformToken.address} />],
                    ["Pool", <AddrLink key="pp" a={treasury.platformToken.pool} />],
                    [L("状态", "Status"), treasury.platformToken.graduated ? <Badge key="g" variant="gold">{t("common.graduated")}</Badge> : <Badge key="c" variant="accent">{t("common.graduating")}</Badge>],
                  ]}
                />
              </>
            )}
          ...
          <Section id="terms" title={L("版本与条款", "Versioning and terms")}>
            ...
            <P>
              {L(
                "链上数据公开免费，如何使用由你负责。ArcLaunch 按现状提供，不作任何担保；团队不对因集成、界面、RPC 或索引器造成的损失负责。引用 ArcLaunch 时请链接回本站，未经书面同意不得暗示合作、背书或官方身份，不得以误导用户的方式使用 ArcLaunch 名称或标识。ArcLaunch 不是 Arc 或 Circle 的官方产品。",
            ...
            <KvGrid rows={[["X", <a key="x" href={X_URL} target="_blank" rel="noreferrer" className="text-primary hover:underline">@{X_HANDLE}</a>]]} />
            <P>
              {L("集成、索引、定价或链上状态问题，请通过 X 私信 ", "For integration, indexing, pricing or onchain-state questions, DM us on X: ")}
