...
不自造组件，统一使用 **shadcn/ui**（GitHub 9 万+ star，Next.js + Tailwind 事实标准，Radix 无障碍基座）：

- `npx shadcn@latest init`（radix-nova 风格）+ `add` 了 Button / Badge / Card / Tabs / Input / Textarea / Label / Progress / Sheet / Dialog / DropdownMenu / Table / Tooltip / Separator / Skeleton / ScrollArea / Sonner / Avatar，源码在 `web/src/components/ui/`，可直接改
- 两套风格都映射到 shadcn 的语义 token（`background / card / primary / muted / accent / border / ring / radius …`），写在 `globals.css` 的 `:root,[data-theme=arc]` 与 `[data-theme=terminal]` 下；shadcn 所有组件自动跟随
- 领域扩展按 shadcn 推荐方式加在生成的组件里：Button 增加 `up / down / gold / glow` 变体和 `xl` 尺寸；Badge 增加 `accent / up / down / gold`；Progress 增加 `indicatorClassName` / `shimmer`
- 额外 token：`--up / --down / --gold`（涨跌/毕业/平台币），暴露为 `text-up` `bg-gold/15` 等
- 主题状态存 `localStorage`，首屏无闪烁（内联脚本预设），URL `?theme=` / `?lang=` 可覆盖
- 业务组件在 `web/src/components/shared.tsx`（Stat / TokenAvatar / TimeAgo / Addr / PctChange …），只组合 shadcn，不写死颜色
- 后续可直接用 shadcn 生态：`shadcn/create` 主题预设、Charts（recharts）、Data Table（tanstack）、Command（搜索面板）
