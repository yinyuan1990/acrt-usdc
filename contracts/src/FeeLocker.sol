...
///
///      Community takeover: the owner cannot move a creator's payout instantly. It proposes a new
///      address, the change becomes executable after `CTO_DELAY`, and the current payout address can
///      veto it during that window.
contract FeeLocker is IERC721Receiver, Ownable, ReentrancyGuard {
    ...
    /// @dev Owner-proposed payout change awaiting the delay.
    mapping(address token => PendingPayout) public pendingPayout;
    ...
    error NoProposal();
    error DelayNotElapsed(uint64 eta);
    ...
    ///         effect immediately. Also clears any owner proposal in flight (acts as a veto).
    function setPayout(address token, address newPayout) external {
        ...
        _clearProposal(token);
        emit PayoutChanged(token, l.payout, newPayout);
        l.payout = newPayout;
    }

    /// @notice Community takeover, step 1 (owner / multisig): propose a new payout address for a token
    ///         whose creator has abandoned it. Nothing changes until `CTO_DELAY` has passed.
    function proposePayout(address token, address newPayout) external onlyOwner {
        Lock storage l = locks[token];
        if (!l.exists) revert UnknownToken();
        if (newPayout == address(0)) revert ZeroAddress();
        uint64 eta = uint64(block.timestamp + CTO_DELAY);
        pendingPayout[token] = PendingPayout({newPayout: newPayout, eta: eta});
        emit PayoutProposed(token, newPayout, eta);
    }

    /// @notice Community takeover, step 2: anyone may apply the proposal once the delay has elapsed.
    function executePayout(address token) external {
        Lock storage l = locks[token];
        if (!l.exists) revert UnknownToken();
        PendingPayout memory p = pendingPayout[token];
        if (p.newPayout == address(0)) revert NoProposal();
        if (block.timestamp < p.eta) revert DelayNotElapsed(p.eta);
        delete pendingPayout[token];
        emit PayoutChanged(token, l.payout, p.newPayout);
        l.payout = p.newPayout;
    }

    /// @notice The current payout address (creator) or the owner can cancel a pending proposal.
    function cancelPayoutProposal(address token) external {
        Lock storage l = locks[token];
        if (!l.exists) revert UnknownToken();
        if (msg.sender != l.payout && msg.sender != owner()) revert NotCreatorOrOwner();
        if (pendingPayout[token].newPayout == address(0)) revert NoProposal();
        _clearProposal(token);
    }

    function _clearProposal(address token) internal {
        if (pendingPayout[token].newPayout != address(0)) {
            delete pendingPayout[token];
            emit PayoutProposalCancelled(token, msg.sender);
        }
    }
