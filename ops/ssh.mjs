 ...
 * Auth:   SSH key at ~/.ssh/arclaunch_ed25519 if present, otherwise SRV_PW env var (bootstrap only).
 * Host:   SRV_HOST (default 45.205.18.104), SRV_USER (default root), SRV_PORT (default 22)
 */
...
const keyPath = join(homedir(), ".ssh", "arclaunch_ed25519");
...
if (!auth.privateKey && !auth.password) {
  console.error("no auth: create ~/.ssh/arclaunch_ed25519 or set SRV_PW");
  process.exit(2);
}
